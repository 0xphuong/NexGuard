...
DOCKER_ROUTE_V4=$(sudo ip -4 route | grep ^$NEXGUARD_DEV_V4)
DEFAULT_ROUTE_V6=$(sudo ip -6 route | grep ^default)
DOCKER_ROUTE_V6=$(sudo ip -6 route | grep ^$NEXGUARD_DEV_V6)
...
sudo ip -4 rule add from $NEXGUARD_DEV_V4 table $TABLE
sudo ip -6 rule add from $NEXGUARD_DEV_V6 table $TABLE
