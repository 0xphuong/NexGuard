...
# rules so that NexGuard can run without a hitch.
#
...
unless node['nexguard']['wireguard']['endpoint']
  ...
  node.consume_attributes('nexguard' => { 'wireguard' => { 'endpoint' => egress_ip } })
end

unless node['nexguard']['egress_interface']
  ...
  node.consume_attributes('nexguard' => { 'egress_interface' => egress_interface })
end
