...
class NexGuard
  ...
  module Config
    ...
    def self.load_or_create!(filename, node)
      ...
      if File.exist?(filename)
        ...
        File.open(filename, 'w') do |file|
          File.open(
            "#{node['nexguard']['install_directory']}/embedded/cookbooks/nexguard/attributes/default.rb", 'r'
    ...
    def self.load_or_create_telemetry_id(filename, node)
      create_directory!(filename)
      if File.exist?(filename)
        node.consume_attributes('nexguard' => { 'telemetry_id' => File.read(filename) })
      else
        telemetry_id = SecureRandom.uuid
        File.open(filename, 'w') do |file|
          file.write telemetry_id
        end
        node.consume_attributes('nexguard' => { 'telemetry_id' => telemetry_id })
      end
    end
    ...
    def self.load_from_json!(filename, node)
      ...
      if File.exist?(filename)
        node.consume_attributes(
          'nexguard' => Chef::JSONCompat.from_json(File.open(filename).read)
        )
      end
    ...
    def self.load_or_create_secrets!(filename, node)
      ...
      node.consume_attributes('nexguard' => secrets)
    ...
    def self.build_secrets(node)
      {
        'guardian_secret_key' => node['nexguard'] && node['nexguard']['guardian_secret_key'] || SecureRandom.base64(48),
        'secret_key_base' => node['nexguard'] && node['nexguard']['secret_key_base'] || SecureRandom.base64(48),
        'live_view_signing_salt' => node['nexguard'] && node['nexguard']['live_view_signing_salt'] || \
          SecureRandom.base64(24),
        'cookie_signing_salt' => node['nexguard'] && node['nexguard']['cookie_signing_salt'] || SecureRandom.base64(6),
        'cookie_encryption_salt' => node['nexguard'] && node['nexguard']['cookie_encryption_salt'] || \
          SecureRandom.base64(6),
        'database_encryption_key' => node['nexguard'] && node['nexguard']['database_encryption_key'] || \
          SecureRandom.base64(32),
        'default_admin_password' => node['nexguard'] && node['nexguard']['default_admin_password'] || \
    ...
    def self.maybe_turn_on_fips(node)
      ...
      case node['nexguard']['fips_enabled']
      when nil
        # the default value, set fips mode based on whether it is enabled in the kernel
        node.normal['nexguard']['fips_enabled'] = NexGuard::Config.fips_enabled_in_kernel?
        if node['nexguard']['fips_enabled']
          Chef::Log.warn('Detected FIPS-enabled kernel; enabling FIPS 140-2 for NexGuard services.')
        end
      when false
        node.normal['nexguard']['fips_enabled'] = NexGuard::Config.fips_enabled_in_kernel?
        if node['nexguard']['fips_enabled']
          Chef::Log.warn('Detected FIPS-enabled kernel; enabling FIPS 140-2 for NexGuard services.')
          Chef::Log.warn('fips_enabled was set to false; ignoring this and setting to true or else NexGuard services'\
                         ' will fail with crypto errors.')
        end
      when true
        Chef::Log.warn('Overriding FIPS detection: FIPS 140-2 mode is ON.')
      else
        node.normal['nexguard']['fips_enabled'] = true
        Chef::Log.warn(
    ...
    def self.app_env(node, reject = [])
      ...
      env = {
        ...
        'WIREGUARD_PRIVATE_KEY_PATH' => "#{node['nexguard']['var_directory']}/cache/wg_private_key",

        'SAML_KEYFILE_PATH' => "#{node['nexguard']['ssl']['directory']}/saml.key",
        'SAML_CERTFILE_PATH' => "#{node['nexguard']['ssl']['directory']}/saml.crt",
