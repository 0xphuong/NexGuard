...
name 'nexguard-cookbooks'
license :project_license

source path: 'cookbooks/nexguard'
...
build do
  ...
  block do
    File.open("#{cookbooks_path}/dna.json", 'w') do |file|
      file.write FFI_Yajl::Encoder.encode(run_list: ['recipe[nexguard::default]'])
    end

    File.open("#{cookbooks_path}/show-config.json", 'w') do |file|
      file.write FFI_Yajl::Encoder.encode(
        run_list: ['recipe[nexguard::show_config]']
      )
    end
    ...
    File.open("#{cookbooks_path}/solo.rb", 'w') do |file|
      file.write <<~SOLO
        ...
        cache_path "/var/opt/nexguard/cache"
