...
### Internal CA + mTLS certs (cert lifecycle)

The `:13443` mTLS listener uses a **separate cert set** from the
`l7_tls_certificates` library described above — different threat
model, different lifecycle, different management surface:

* **Generated by**: `scripts/l7-rotate-proxy-cert.sh` (host-side
  openssl, no Phoenix involvement)
* **Stored at**: `$NEXGUARD_CERTS_DIR` (default
  `/var/nexguard/l7-certs/`) on the host filesystem — **not** in
  Postgres
* **Validity**: Internal CA root = 10 years; server + proxy leaf
  certs = 1 year
* **Visible in admin UI?** No. `TlsCertExpiryScanner`
  (`FzHttp.L7.TlsCertExpiryScanner`) only scans DB-stored library
  certs — internal CA certs are an **observability blind spot**;
  operators must self-monitor via cron + openssl, or set a
  calendar reminder

Full rotation procedure (annual leaf rotation, decennial CA
rotation, verification commands, downtime scope) lives in
[`docs/migrations/v3.0.0.md`](docs/migrations/v3.0.0.md) under
"Cert lifecycle".
