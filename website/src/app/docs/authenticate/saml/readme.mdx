...
`SAML_KEYFILE_PATH=/var/nexguard/saml.key` and
`SAML_CERTFILE_PATH=/var/nexguard/saml.crt` environment variables for the
Firezone container.
...
variable and defaults to `urn:nexguard.dev:firezone-app` if not set.
